...
export class Sanctuary extends Obstacle {
    safeAreaRadius;
    constructor(game, x, y) {
        super(game, x, y);
    ...
    draw() {
        super.draw();
        this.game.ctx.beginPath();
        this.game.ctx.arc(this.spriteX + this.width * 0.5, this.spriteY + this.height * 0.5, this.safeAreaRadius, 0, Math.PI * 2);
        this.game.ctx.save();
    ...
    update() {
        ...
        collisionObjects.forEach((object) => {
            const { distance, dx, dy, isCollision, sumOfRadius } = this.checkCollisionSafeArea(object);
            if (isCollision) {
